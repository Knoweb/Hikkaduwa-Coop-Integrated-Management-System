...
const GoodsReceivedNote: React.FC = () => {
  ...
  const [items, setItems] = useState<GrnItemRequest[]>([{
    beerItemId: '',
    quantity: '',
    unitCost: ''
  }]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [supplierRes, catalogRes] = await Promise.all([api.get('/api/v1/beer-garden/suppliers'), api.get('/api/v1/beer-garden/items')]);
        setSuppliers(supplierRes.data);
        const sortedCatalog = catalogRes.data.sort((a: BeerItem, b: BeerItem) => a.beerName.localeCompare(b.beerName));
  ...
  return <Box sx={{
    ...
    mx: 'auto'
      ...
      fontWeight: 'bold',
      mb: 3
    }}>Purchase Procurement (GRN)</Typography>
    ...
    {canMutateBusinessData ? (
      ...
      mb: 3
          ...
          md: 4
                        ...
                        <TextField select fullWidth label="Payment Method" value={paymentMethod} onChange={e => setPaymentMethod(e.target.value)}>
                            {canMutateBusinessData && <MenuItem value="CREDIT">Credit (Add to Supplier Debt)</MenuItem>}
      ...
      mb: 3
        ...
        mb: 2,
        fontWeight: 'bold'
      }}>Procurement Items</Typography>
                {items.map((item, index) => <Grid container spacing={2} key={index} sx={{
        alignItems: 'center',
        mb: 2
      }}>
                        <Grid size={{
          xs: 4
                            ...
                            <TextField select fullWidth label="Select Catalog Item" size="small" value={item.beerItemId} onChange={e => handleItemChange(index, 'beerItemId', e.target.value)}>
                                ...
                                {beerCatalog.map(beer => <MenuItem key={beer.id} value={beer.id}>{beer.beerName}</MenuItem>)}
                            </TextField>
                        </Grid>
                        <Grid size={{
          xs: 3
          ...
          textAlign: 'right'
            ...
            mr: 2
                ...
                {canMutateBusinessData && <Button onClick={addItemRow} sx={{
        mt: 1,
        fontWeight: 'bold'
      ...
      borderRadius: 2
                ...
                <Typography variant="h5">Grand Total: Rs. {calculateTotal().toLocaleString(undefined, {
          minimumFractionDigits: 2
                ...
                <Typography color="text.secondary">You do not have permission to create purchase procurements. Please view the Purchase History page.</Typography>
            )}
        </Box>;
};
