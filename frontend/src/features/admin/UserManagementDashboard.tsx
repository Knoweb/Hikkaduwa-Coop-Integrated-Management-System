...
import { Box, Typography, Button, TextField, MenuItem, Grid, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Alert, Chip, IconButton, Tooltip, InputAdornment, Dialog, DialogTitle, DialogContent, DialogActions, DialogContentText } from '@mui/material';
  ...
  const formatRole = (backendRole: string) => {
    switch (backendRole) {
      case 'ROLE_ADMIN':
        return 'Global Admin';
      case 'ROLE_MILK_SHOP':
        return 'Milk Shop Operator';
      case 'ROLE_BEER_GARDEN':
        return 'Beer Garden Manager';
      case 'ROLE_ROOM_BOOKING':
        return 'Room Section Manager';
      case 'ROLE_AUDITOR':
        return 'Auditor';
      default:
        return backendRole;
    ...
    mx: 'auto'
      ...
      color: '#1e293b'
            ...
            <Grid container spacing={4}>
                {/* Form Column */}
                {canMutateBusinessData && <Grid size={{
        xs: 12,
        md: 4
      }}>
                    <Paper sx={{
          ...
          boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)'
            ...
            mb: 3,
            fontWeight: 'bold'
          }}>Provision New Account</Typography>
                        {message && <Alert severity={message.type} sx={{
            mb: 3
          }}>{message.text}</Alert>}

                        <form onSubmit={handleCreateUser}>
                            <TextField fullWidth label="Full Name" size="small" sx={{
              mb: 2
            }} value={name} onChange={e => setName(e.target.value)} required />

                            <TextField fullWidth label="Username" size="small" sx={{
              mb: 2
            }} value={username} onChange={e => setUsername(e.target.value)} required />

                            <TextField fullWidth type={showPassword ? 'text' : 'password'} label="New Password" size="small" sx={{
              mb: 2
            }} value={rawPassword} onChange={e => setRawPassword(e.target.value)} required slotProps={{
              input: {
                endAdornment: <InputAdornment position="end">
                    <IconButton onClick={() => setShowPassword(!showPassword)} edge="end">
                        {showPassword ? <VisibilityOff /> : <Visibility />}
                            ...
                            <TextField select fullWidth label="System Role" size="small" sx={{
              mb: 3
                            ...
                            <Button type="submit" variant="contained" fullWidth size="large" disabled={isLoading || !isFormValid} startIcon={<PersonAddIcon />} sx={{
              py: 1.5,
              fontWeight: 'bold',
          ...
          boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)'
            ...
            maxHeight: 600
                    ...
                    bgcolor: '#f1f5f9'
                                        ...
                                        {canMutateBusinessData && <TableCell align="center" sx={{
                    fontWeight: 'bold',
                    bgcolor: '#f1f5f9'
                  }}>Action</TableCell>}
                                    </TableRow>
            ...
            <Dialog open={resetDialogOpen} onClose={() => setResetDialogOpen(false)} maxWidth="sm" fullWidth>
                <DialogTitle sx={{
        fontWeight: 'bold'
      }}>Change Password for '{selectedUserName}'</DialogTitle>
          ...
          mb: 3
                    ...
                    <TextField autoFocus fullWidth label="New Password" type="text" value={newPassword} onChange={e => setNewPassword(e.target.value)} required />
                </DialogContent>
                <DialogActions sx={{
        p: 2
      }}>
                    {canMutateBusinessData && <Button onClick={() => setResetDialogOpen(false)} color="inherit">Cancel</Button>}
                    <Button onClick={handleResetPassword} variant="contained" color="primary" disabled={newPassword.length < 6 || isLoading}>
